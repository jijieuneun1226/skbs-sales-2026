...
<style>
    ...
    /* [수정] 글자색 강제 고정 및 가독성 향상 */
    .summary-title { color: #1A3A5F !important; font-weight: 800; font-size: 19px; margin-top: 15px; margin-bottom: 10px; border-bottom: 1px solid #eee; }
    .insight-title { color: #D97706 !important; font-weight: 800; font-size: 19px; margin-top: 15px; margin-bottom: 10px; border-bottom: 1px solid #eee; }
    .content-item { color: #333333 !important; font-size: 15px; line-height: 1.8; margin-bottom: 5px; padding-left: 10px; }
    .content-item b { color: #000000 !important; font-weight: 700; }
</style>
""", unsafe_allow_html=True)

st.title("📊 SKBS Sales Report")

# URL 축약 대응형 파라미터 로드 함수
def get_p(key, default, df_full=None, col=None):
    res = params.get_all(key)
    if not res: return default
    # 'all' 키워드가 들어있으면 전체 리스트 반환 (축약 대응)
...
def load_data_from_drive(file_id):
    ...
    try:
        ...
        def classify_channel(group): return 'online' if str(group) in ['B2B', 'B2B(W)', 'SAP', '의사회원'] else 'offline'
...
if is_edit_mode:
    with st.sidebar:
        ...
        if st.button("🔗 축약 공유 링크 생성"):
            ...
            c_encoded = "&c=".join([urllib.parse.quote(x) for x in sel_channels])
            
            # http://www.druginfo.co.kr/detail/sideEffect.aspx?pid=5070 제품군/제품명이 전체 선택인 경우 'all'로 축약하여 전송
            ...
            p_string = (f"?y={'&y='.join(map(str, sel_years))}&c={c_encoded}&q={'&q='.join(map(str, sel_quarters))}"
                        f"&m={'&m='.join(map(str, sel_months))}&cat={cat_p}&prod={prod_p}")
            st.success("링크 생성 완료! (축약형)"); st.code(base_url + p_string, language="text")
...
def render_smart_overview(df_curr, df_raw):
    ...
    current_year = int(df_curr['년'].max()); last_year = current_year - 1
...
def render_winback_quality(df_final, df_raw, current_year):
    ...
    c1.metric("재유입 거래처", f"{len(wb_ids)} 처"); c2.metric("확보 매출", f"{df_wb_l['올해'].sum():,.0f} 백만원"); c3.metric("평균 회복률", f"{df_wb_l['회복률'].mean():.1f}%")
    ...
    with col_b:
        st.markdown('<p class="guide-text">💡 행 클릭 시 상세 공백일수 확인 가능</p>', unsafe_allow_html=True)
    ...
    if len(event_wb.selection.rows) > 0:
        ...
        detail = df_h[df_h['사업자번호'] == sel_biz].sort_values('매출일자', ascending=False)
        st.dataframe(detail[['매출일자', '제품명', '매출액', '수량', '구매간격']].rename(columns={'구매간격':'공백일수'}), use_container_width=True)
...
with tab2:
    if not df_final.empty:
        ...
        st.markdown("<div class='fix-title-blue'>📊 데이터 요약</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='fix-text'>• 상위 100대 매출 합계: <b>{top100_v['매출액'].sum():,.0f} 백만원</b> (비중 {(top100_v['매출액'].sum()/total_s*100):.1f}%)<br>• 상태 분포: <b>기존({st_c.get('✅ 기존 (유지)',0)}처), 신규({st_c.get('🆕 신규 (New)',0)}처), 재유입({st_c.get('🔄 재유입 (복귀)',0)}처)</b><br>• 최고 매출 거래처: <b>{top100_v.iloc[0]['거래처명']}</b></div>", unsafe_allow_html=True)
        st.markdown("<div class='fix-title-orange'>💡 스마트 인사이트</div>", unsafe_allow_html=True)
    ...
    st.markdown('<p class="guide-text">💡 행 클릭 시 상세 실적 표시</p>', unsafe_allow_html=True)
...
with tab4:
    ...
    st.markdown("<div class='fix-title-blue'>📊 데이터 요약</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='fix-text'>• 최고 매출: <b>{reg_v.iloc[0]['지역']}</b> ({reg_v.iloc[0]['Sales']:,.0f} 백만원) / 최다 거래처: <b>{reg_v.sort_values('Count', ascending=False).iloc[0]['지역']}</b></div>", unsafe_allow_html=True)
    
    st.markdown("### 🗺️ 지역별 실적 리스트")
    ...
    with col_r2: st.plotly_chart(px.pie(df_final[df_final['지역'] == sel_r], values='매출액', names='제품명', hole=0.3, title=f"[{sel_r}] 제품 비중"), use_container_width=True)
...
with tab5:
    ...
    st.markdown("<div class='fix-title-blue'>📊 데이터 요약</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='fix-text'>• 최다 판매: <b>{p_v.iloc[0]['제품명']}</b> / 최대 매출 제품군: <b>{cat_v.iloc[0]['제품군']}</b></div>", unsafe_allow_html=True)
