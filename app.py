    ...
    try:
        ...
        for sn in ['Brand_Monthly', 'Brand_Total', 'Brand_Direct_Sales']:
            if sn in sheets:
                ...
                brand_data[sn] = b_df
            else: brand_data[sn] = pd.DataFrame()
...
def render_winback_quality(df_final, df_raw, current_year):
    ...
    if len(winback_ids) == 0: st.info("♻️ 해당 조건 내 재유입 데이터(6개월 공백 기준)가 없습니다."); return
    ...
    if len(event_wb.selection.rows) > 0:
        ...
        st.dataframe(detail_hist[['매출일자_str', '제품명', '매출액', '수량', '구매간격']].rename(columns={'매출일자_str':'매출일자', '구매간격':'직전구매후공백(일)'})
                     ...
                     .format({'매출액': '{:,.1f} 백만원', '직전구매후공백(일)': '{:,.0f} 일'}), use_container_width=True)
...
def render_regional_deep_dive(df):
    ...
    st.plotly_chart(px.scatter(reg_stats, x='Count', y='Per', size='Sales', color='지역', text='지역', labels={'Count': '거래처 수', 'Per': '평균 객단가 (백만원)'}), use_container_width=True)
    c1, c2 = st.columns(2)
...
def render_brand_store_analysis(brand_data, sel_years, df_raw):
    ...
    with col_l:
        ...
        if not df_m.empty: st.plotly_chart(px.line(df_m, x='월', y=['UV', 'PV'], markers=True, color_discrete_sequence=['#4e79a7', '#e15759']), use_container_width=True)
    with col_r:
    ...
    if not df_d.empty:
        top5 = df_d.groupby('상품명_매핑').agg(매출액=('매출_백만', 'sum'), 구매수량=('수량', 'sum')).sort_values('매출액', ascending=False).head(5)
...
with tab2:
    if not df_final.empty:
        ...
        if len(event_cls.selection.rows) > 0:
            ...
            st.dataframe(df_raw[df_raw['사업자번호'] == row_biz].sort_values('매출일자', ascending=False).head(20).style.format({'매출액': '{:,.1f} 백만원'}), use_container_width=True)
