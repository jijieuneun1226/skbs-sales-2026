...
# --------------------------------------------------------------------------------
# 3. [추가] 신규 분석 함수들 (ValueError 방지 및 단위 표기 적용)
# --------------------------------------------------------------------------------
...
# --- [TAB 3] 재유입 패턴 ---
with tab3:
    render_winback_quality(df_raw, sel_years[0]) # 🌟 신규 상단 추가
    st.markdown("---")
    st.markdown("### 🔄 재유입 제품별 실적 및 이탈 전 패턴 (기본 틀)")
    ...
    if not res.empty:
        ...
        with c_rl: ev_res = st.dataframe(res_sum.style.format({'총_재유입매출': '{:,.1f}백만원'}), use_container_width=True, on_select="rerun", selection_mode="single-row", height=400)
        with c_rr:
...
# --- [TAB 4] 지역 분석 ---
with tab4:
    render_regional_deep_dive(df_final) # 🌟 신규 상단 추가
    st.markdown("---")
    st.markdown("### 🗺️ 지역별 실적 및 심층 내역 (기본 틀)")
    ...
    with c_r2:
        if not reg_s.empty: st.plotly_chart(px.scatter(reg_s, x='구매처수', y='매출액', text='지역', size='마커크기', color='매출액'), use_container_width=True)
    if sel_reg:
        st.markdown("---")
        reg_df = df_final[df_final['지역'] == sel_reg]
        col_reg_a, col_reg_b = st.columns(2)
        with col_reg_a: st.plotly_chart(px.pie(reg_df, values='매출액', names='제품명', hole=0.3, title=f"[{sel_reg}] 제품 비중"), use_container_width=True)
        with col_reg_b:
...
# --- [TAB 5] 제품 분석 ---
with tab5:
    render_product_strategy(df_final) # 🌟 신규 상단 추가
    st.markdown("---")
    st.markdown("### 📦 제품별 판매 현황 및 고객 상세 분석 (기본 틀)")
    ...
    if t5_list:
        ...
        st.plotly_chart(px.line(tr_df, x='년월', y='매출액', color='제품명', title="Top 5 제품 월별 매출 추이"), use_container_width=True)
