...
# --------------------------------------------------------------------------------
# 3. [SK분석 기본 폼] 분석 함수 정의 (성역: 사용자 원본 그대로 유지)
# --------------------------------------------------------------------------------
...
def render_regional_deep_dive(df):
    ...
    with c2:
        ...
        for r in df['지역'].unique():
            r_df = df[df['지역'] == r]
            risk_val = (r_df.groupby('거래처명')['매출액'].sum().max() / r_df['매출액'].sum() * 100)
            risk.append({'지역': r, '의존도': risk_val})
...
def render_product_strategy(df):
    ...
    with c2: st.plotly_chart(px.bar(p_stats.sort_values('Penetration' if 'Penetration' in p_stats else 'Count'), x=(p_stats['Count'] / df['사업자번호'].nunique() * 100), y='제품명', orientation='h', text_auto='.1f', title="시장 침투율 (%)"), use_container_width=True)
    ...
    with st.expander("🧩 함께 팔기(Cross-selling) 기회 분석기", expanded=True):
        ...
        with col_s1: base_p = st.selectbox("Anchor 제품 (이미 쓰는 것)", all_p, index=0)
        with col_s2: target_p = st.selectbox("Target 제품 (팔고 싶은 것)", all_p, index=min(1, len(all_p)-1))
        if base_p != target_p:
...
with tab2:
    if not df_final.empty:
        ...
        st.write(f"• **VIP 이탈 위험 알림:** 현재 상위 100처 거래처 중 **{risk_cnt}처**가 90일 이상 구매가 없는 이탈 위험 상태입니다. 특히 **{top_v['거래처명']}**의 관리가 시급합니다.")

        st.markdown("---")
        st.markdown("### 🏆 상위 거래처 상세 분석")
        # [수정] 매출액 단위 백만원 고정 및 서식 적용
        event_vip = st.dataframe(ranking_v[['거래처명', '진료과', '매출액']].rename(columns={'매출액':'매출액(백만원)'}), 
        ...
        if len(event_vip.selection.rows) > 0:
            ...
            v_biz = ranking_v.iloc[v_idx]['사업자번호']
            # [수정] 클릭 시 '수량' 컬럼 추가 및 백만원 서식 적용
        ...
        if len(event_cls.selection.rows) > 0:
            ...
            st.dataframe(df_raw[df_raw['사업자번호'] == row_biz].sort_values('매출일자', ascending=False).head(20).style.format({'매출액': '{:,.1f} 백만원', '수량': '{:,}'}), use_container_width=True)
...
# [복구 및 고정] 🏠 6. 브랜드관 성과 분석 (누락 차트 전체 복구)
with tab6:
    ...
    if brand_data_dict['Brand_Total'].empty:
        ...
        if not df_m.empty: df_m = df_m[df_m['월'].astype(str).str.startswith(str(t_year))]
        ...
        with st.container(border=True):
            ...
            with c2:
                ...
                if uv > 0: st.progress(count / uv, text=f"전환율 {(count/uv*100):.1f}%")
            with c3:
        ...
        st.table(pd.DataFrame({"구분": ["UV (방문자수)", "브랜드관 전환 매출액", "구매 전환 처수", "객단가 (ATV)"], 
                               "성과 지표": [f"{uv:,} 명", f"{sales_m:,.1f} 백만원", f"{count:,} 처", f"{atv:,.0f} 원"]}))
