...
# --------------------------------------------------------------------------------
# 2. 데이터 로드 및 전처리 (시트명 'SKBS' 및 지역 컬럼 오류 해결)
# --------------------------------------------------------------------------------
...
def render_winback_quality(df_final, df_raw, current_year):
    ...
    st.markdown("""<div class="info-box"><b>🔍 재유입 정의:</b> 직전 구매일로부터 <b>최소 180일(6개월) 이상 공백기</b> 이후 다시 구매가 발생한 거래처 (선택 기간 내 첫 구매 기준)</div>""", unsafe_allow_html=True)
    ...
    if len(winback_ids) == 0: st.info("♻️ 해당 조건 내 재유입 데이터가 없습니다."); return
...
def render_brand_store_analysis(brand_data, sel_years, df_raw):
    ...
    if not brand_data or brand_data['Brand_Total'].empty:
        st.warning("🏠 브랜드관 분석 데이터가 없습니다. 시트 구성을 확인해 주세요."); return

    # 1. 제품명 매핑 (Brand -> Sales 기준)
    valid_p = df_raw['제품명'].unique()
    def map_p(name):
        clean_name = str(name).replace('[SK]', '').replace('주', '').replace('0.5ml', '').strip()
        for vp in valid_p:
    ...
    if not df_direct.empty:
        ...
        df_direct['상품명_매핑'] = df_direct['상품명'].apply(map_p)
    ...
    conv_sales = (df_direct['매출'].sum() / 1000000) if not df_direct.empty else 0
    conv_count = df_direct['사업자번호'].nunique() if not df_direct.empty else 0
    atv = (conv_sales * 1000000 / conv_count) if conv_count > 0 else 0

    # 4. 상단 Summary (1번탭 디자인과 유사하게 구성)
    st.markdown(f"### 🚀 브랜드관 성과 Summary ({target_year}년)")
    st.markdown("""<div class="info-box"><b>💡 분석 기준:</b> 브랜드관 페이지 유입 고객의 활동성과 실제 구매 전환(당일 구매)을 분석합니다.</div>""", unsafe_allow_html=True)
    with st.container(border=True):
        c1, c2, c3 = st.columns([1.2, 1, 1.2])
        with c1:
    ...
    perf_df = pd.DataFrame({
        ...
        "성과 지표": [f"{uv:,} 명", f"{conv_sales:,.1f} 백만원", f"{conv_count:,} 처", f"{atv:,.0f} 원"]
    })
    st.table(perf_df)
    ...
    with col_chart:
        ...
        if not df_monthly.empty:
            fig_m = px.line(df_monthly, x='월', y=['UV', 'PV'], markers=True, color_discrete_map={'UV':'#4e79a7', 'PV':'#e15759'})
    ...
    if not df_direct.empty:
        top5 = df_direct.groupby('상품명_매핑').agg(매출액=('매출', lambda x: x.sum()/1000000), 구매수량=('수량', 'sum')).sort_values('매출액', ascending=False).head(5)
