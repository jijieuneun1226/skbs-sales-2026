...
<style>
    ...
    /* 인사이트 섹션 스타일 최적화 (배경색 제거) */
    .section-container { padding: 0px; margin-bottom: 20px; }
    .section-title { font-weight: 800; font-size: 1.1rem; color: #1f2937; margin-bottom: 5px; display: flex; align-items: center; }
    .section-list { list-style-type: disc; margin-left: 20px; font-size: 14px; line-height: 1.6; }
</style>
...
def render_smart_overview(df_curr, df_raw):
    ...
    with st.container(border=True):
        ...
        with c2:
            ...
            if len(cust_curr) > 0: st.progress(len(cust_curr & cust_prev) / len(cust_curr), text=f"고객 유지율 {(len(cust_curr & cust_prev)/len(cust_curr))*100:.1f}%")
        with c3:
...
def render_winback_quality(df_final, df_raw, current_year):
    ...
    with col_li:
        st.markdown('<p class="guide-text">💡 행 클릭 시 하단에서 실제 공백 기간 확인 가능</p>', unsafe_allow_html=True)
    ...
    if len(event_wb.selection.rows) > 0:
        sel_biz_no = df_wb.iloc[event_wb.selection.rows[0]]['사업자번호']
        detail_hist = df_history[df_history['사업자번호'] == sel_biz_no].sort_values('매출일자', ascending=False).copy()
        detail_hist['매출일자'] = detail_hist['매출일자'].dt.strftime('%Y-%m-%d')
        st.dataframe(detail_hist[['매출일자', '제품명', '매출액', '수량', '구매간격']].rename(columns={'구매간격':'직전구매후공백(일)'})
                     .style.applymap(lambda v: 'background-color: #ffcccc;' if isinstance(v, (int, float)) and v >= 180 else '', subset=['직전구매후공백(일)'])
...
def render_regional_deep_dive(df):
    ...
    - <b>핵심 거점 의존도:</b> 지역 내 1위 거래처가 차지하는 매출 비중
    </div>""", unsafe_allow_html=True)
...
def render_product_strategy(df):
    ...
    with c2:
        ...
        st.plotly_chart(px.bar(p_stats.sort_values('Penetration'), x='Penetration', y='제품명', orientation='h', text_auto='.1f', title="시장 침투율 (%)"), use_container_width=True)
    
    season_pivot = df.pivot_table(index='제품명', columns='월', values='매출액', aggfunc='sum', fill_value=0)
    st.plotly_chart(px.imshow(season_pivot.div(season_pivot.max(axis=1), axis=0), color_continuous_scale="Reds", aspect="auto", title="제품별 판매 시즌 집중도"), use_container_width=True)
...
with tab2:
    # 탭 2 요약 및 인사이트 (배경색 제거)
    if not df_final.empty:
        total_s = df_final['매출액'].sum()
        top100_data = df_final.groupby(['사업자번호', '거래처명', '진료과']).agg({'매출액': 'sum'}).sort_values('매출액', ascending=False).head(100).reset_index()
        top100_sum = top100_data['매출액'].sum()
        last_p = df_raw.groupby('사업자번호')['매출일자'].max()
        top100_data['최근구매일'] = top100_data['사업자번호'].map(last_p)
        cur_d = df_raw['매출일자'].max()
        risk_cnt = len(top100_data[(cur_d - top100_data['최근구매일']).dt.days >= 90])
        cls_data = classify_customers(df_raw, sel_years[0])
        st_counts = cls_data['상태'].value_counts()

        st.markdown(f"""
        <div class="section-container">
            <div class="section-title">📊 데이터 요약</div>
            <div class="section-list">
                <li><b>상위 100대 매출 합계:</b> {top100_sum:,.0f} 백만원 (전체 매출의 {(top100_sum/total_s*100):.1f}% 차지)</li>
                ...
                <li><b>평균 객단가:</b> 처당 약 {(total_s / df_final['사업자번호'].nunique() * 100):,.0f} 만원</li>
                <li><b>최고 매출 거래처:</b> {top100_data.iloc[0]['거래처명']} ({top100_data.iloc[0]['매출액']:,.0f} 백만원)</li>
            </div>
        </div>
        <div class="section-container">
            <div class="section-title">💡 스마트 인사이트</div>
            <div class="section-list">
                <li><b>VIP 이탈 위험 알림:</b> 현재 상위 100대 거래처 중 <b>{risk_cnt}처({risk_cnt}%)</b>가 90일 이상 구매가 없는 이탈 위험 상태입니다. 특히 <b>{top100_data.iloc[0]['거래처명']}</b>의 공백이 관리 필요 대상입니다.</li>
                <li><b>신규 vs 이탈 밸런스:</b> 금기 신규 유입은 {st_counts.get('🆕 신규 (New)', 0)}처이며, 이탈은 {st_counts.get('📉 1년 이탈', 0)}처입니다. 유입 대비 이탈 관리에 집중해야 합니다.</li>
                <li><b>진료과 집중도 리스크:</b> 매출의 {(df_final.groupby('진료과')['매출액'].sum().max()/total_s*100):.1f}%가 <b>{df_final.groupby('진료과')['매출액'].sum().idxmax()}</b>에 편중되어 있어 타 진료과 확장이 권장됩니다.</li>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.expander("🥇 매출 상위 거래처 Top 100 리스트", expanded=True):
        top100_data['최근구매일_str'] = top100_data['최근구매일'].dt.strftime('%Y-%m-%d')
        top100_data['상태'] = (cur_d - top100_data['최근구매일']).dt.days.apply(lambda x: '🚨 이탈위험' if x >= 90 else '✅ 정상')
        event_vip = st.dataframe(top100_data[['상태', '거래처명', '진료과', '매출액', '최근구매일_str']].style.format({'매출액': '{:,.1f} 백만원'}), use_container_width=True, on_select="rerun", selection_mode="single-row", height=350)
        if len(event_vip.selection.rows) > 0:
            sel_v_idx = top100_data.index[event_vip.selection.rows[0]]
            st.dataframe(df_raw[df_raw['사업자번호'] == top100_data.loc[sel_v_idx, '사업자번호']].groupby('제품명').agg({'매출액': 'sum'}).sort_values('매출액', ascending=False).style.format({'매출액': '{:,.1f} 백만원'}), use_container_width=True)
    st.markdown("---")
    c_s1, c_s2 = st.columns([1, 2])
    with c_s1:
        st.dataframe(cls_data['상태'].value_counts().reset_index().rename(columns={'count':'거래처수'}), use_container_width=True)
        sel_st = st.selectbox("👇 분석할 그룹 선택", sorted(cls_data['상태'].unique()), key="p2_sel")
    with c_s2: st.plotly_chart(px.pie(cls_data[cls_data['상태'] == sel_st], names='진료과', title="진료과 분포"), use_container_width=True)
    display_cls = cls_data[cls_data['상태'] == sel_st].sort_values('해당년도_매출', ascending=False).copy()
...
with tab3:
    ...
    if not wb_v.empty:
        ...
        <div class="section-container">
            <div class="section-title">📊 데이터 요약</div>
            <div class="section-list">
                <li><b>총 재유입 거래처:</b> {len(wb_ids_l)} 처 (6개월 공백 후 복귀 기준)</li>
                <li><b>재유입 발생 총 매출:</b> {df_wb_l['올해'].sum():,.0f} 백만원</li>
                <li><b>평균 회복률:</b> {df_wb_l['회복률'].mean():.1f}% (과거 최고 매출 대비 현재 비율)</li>
                <li><b>그룹별 분포:</b> 완전 회복({len(df_wb_l[df_wb_l['회복률'] >= 80])}처), 회복 중({len(df_wb_l[(df_wb_l['회복률']<80)&(df_wb_l['회복률']>=20)])}처), 재진입 초기({len(df_wb_l[df_wb_l['회복률'] < 20])}처)</li>
            </div>
        </div>
        <div class="section-container">
            <div class="section-title">💡 스마트 인사이트</div>
            <div class="section-list">
                <li><b>회복 퀄리티 요약:</b> 올해 재유입된 거래처의 평균 회복률은 <b>{df_wb_l['회복률'].mean():.1f}%</b>입니다. 복귀 고객의 충성도가 서서히 정상화되고 있습니다.</li>
                <li><b>복귀 트리거 제품:</b> 이탈 고객들이 복귀 시 가장 먼저 찾는 제품은 <b>{trig_pv}</b>입니다.</li>
                <li><b>재유입 매출 기여도:</b> 전체 매출 중 재유입 거래처가 기여하는 비중은 <b>{(df_wb_l['올해'].sum()/df_final['매출액'].sum()*100):.1f}%</b>입니다.</li>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
with tab4:
    ...
    if not df_final.empty:
        ...
        comp_v['Diff_S'] = comp_v['Sales'] - comp_v['LY_S']
        ...
        <div class="section-container">
            <div class="section-title">📊 데이터 요약</div>
            <div class="section-list">
                <li><b>최다 거래 지역:</b> {top_c_v['지역']} ({top_c_v['Count']}처)</li>
                <li><b>최고 매출 지역:</b> {top_s_v['지역']} ({top_s_v['Sales']:,.0f} 백만원)</li>
                <li><b>거래처 변동:</b> 전년 대비 {inc_r['지역']} {int(inc_r['Diff_C'])}처 증가 / {dec_r['지역']} {abs(int(dec_r['Diff_C']))}처 감소</li>
                <li><b>매출 변동:</b> 전년 대비 {top_s_v['지역']}은 매출 흐름이 안정적/변동적입니다.</li>
            </div>
        </div>
        <div class="section-container">
            <div class="section-title">💡 스마트 인사이트</div>
            <div class="section-list">
                <li><b>커버리지 변동 (YoY):</b> 현재 <b>{inc_r['지역']}</b>은 전년 대비 거래처 수가 증가하며 활발히 확장 중입니다. 반면 <b>{dec_r['지역']}</b>은 인력 재배치가 검토됩니다.</li>
                <li><b>매출 등락 분석:</b> {dec_r['지역']}은 거래처 수 변동과 매출 하락의 상관관계를 분석하여 객단가 개선이 필요합니다.</li>
                <li><b>핵심 거점 의존도:</b> 특정 지역의 매출 비중이 상위 1개 병원에 집중되어 있는지 확인하여 리스크를 분산해야 합니다.</li>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
with tab5:
    # 탭 5 데이터 요약 및 인사이트 (배경색 제거)
    if not df_final.empty:
        p_v = df_final.groupby('제품명').agg(Sales=('매출액', 'sum'), Qty=('수량', 'sum'), Count=('사업자번호', 'nunique')).reset_index().sort_values('Sales', ascending=False)
        top_pv_l = p_v.iloc[0]
        cat_v_l = df_final.groupby('제품군')['매출액'].sum().reset_index()
        top_cv_l = cat_v_l.loc[cat_v_l['매출액'].idxmax()]

        st.markdown(f"""
        <div class="section-container">
            <div class="section-title">📊 데이터 요약</div>
            <div class="section-list">
                <li><b>최다 판매 제품:</b> {top_pv_l['제품명']} ({int(top_pv_l['Qty']):,}개 / {top_pv_l['Sales']:,.0f} 백만원)</li>
                <li><b>성장률 1위 제품:</b> 데이터 기반 추적 중</li>
                <li><b>최대 매출 제품군:</b> {top_cv_l['제품군']} (전체 매출의 {(top_cv_l['매출액']/df_final['매출액'].sum()*100):.1f}% 차지)</li>
                <li><b>시장 침투율:</b> 전체 거래처 중 {(p_v['Count'].sum()/df_final['사업자번호'].nunique()*100):.1f}%가 제품 구매 중</li>
            </div>
        </div>
        <div class="section-container">
            <div class="section-title">💡 스마트 인사이트</div>
            <div class="section-list">
                <li><b>제품별 판매 요약:</b> <b>{top_pv_l['제품명']}</b>이 전체 실적을 견인하고 있으며, '성숙기' 단계 진입 여부를 모니터링해야 합니다.</li>
                <li><b>제품군별 기여도:</b> <b>{top_cv_l['제품군']}</b>의 기여도가 높으며, 신규 제품군의 채택률을 높이는 전략이 유효합니다.</li>
                <li><b>교차 판매 기회:</b> 하단 크로스셀링 분석기를 통해 연관 제품 미구매 병원을 발굴하세요.</li>
            </div>
        </div>
        """, unsafe_allow_html=True)
