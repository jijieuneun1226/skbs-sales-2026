...
def load_data_from_drive(file_id):
    ...
    try:
        ...
        # [수정] 멀티 시트 대응을 위해 ExcelFile 사용 (원본 df 유지)
    ...
    return df, brand_data # brand_data 추가 반환

# --------------------------------------------------------------------------------
# 3. [SK분석 기본 폼] 분석 함수 정의 (성역: 사용자 원본 100% 동일)
# --------------------------------------------------------------------------------
...
def render_brand_store_analysis(brand_data, sel_years, df_raw):
    ...
    if not df_monthly.empty: df_monthly = df_monthly[df_monthly['월'].str.startswith(str(target_year))]

    uv = df_total['UV'].sum() if not df_total.empty else 0
    pv = df_total['PV'].sum() if not df_total.empty else 0
    sales_m = df_direct['매출_백만'].sum() if not df_direct.empty else 0
    count = df_direct['사업자번호'].nunique() if not df_direct.empty else 0
    ...
    with col_l:
        ...
        if not df_monthly.empty: st.plotly_chart(px.line(df_monthly, x='월', y=['UV', 'PV'], markers=True), use_container_width=True)
    with col_r:
        st.markdown("#### 🏥 전환 고객 진료과 비중")
        if not df_direct.empty: st.plotly_chart(px.pie(df_direct, values='매출', names='진료과', hole=0.4), use_container_width=True)

# --------------------------------------------------------------------------------
# 4. 필터 및 실행 (성역: 사용자 원본 100% 동일)
# --------------------------------------------------------------------------------
DRIVE_FILE_ID = "1lFGcQST27rBuUaXcuOJ7yRnMlQWGyxfr"
df_raw, brand_data_dict = load_data_from_drive(DRIVE_FILE_ID) # 반환값 수정 반영
...
# --------------------------------------------------------------------------------
# 5. 메인 탭 구성 (성역: 사용자 원본 100% 동일 + 탭 6만 추가)
# --------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
# [추가] 🏠 6. 브랜드관 성과 분석 (사용자 원본 하단에 깔끔하게 추가)
# --------------------------------------------------------------------------------
with tab6:
